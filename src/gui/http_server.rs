...
impl GUITrait for GUI {
    fn run(mut gc: GC) {
        let threads = [
            ...
            thread::spawn(move || {
                let mut server =
                    websocket::server::sync::Server::bind(format!("127.0.0.1:{}", WS_PORT))
                        .unwrap();
                ...
                while let Ok(mut stream) = server.accept() {
                    ...
                    drop(gc);
                    let initial_msg_bits =
                        bitcode::serialize(&ServerMsg::Initial(initial_msg)).unwrap();
